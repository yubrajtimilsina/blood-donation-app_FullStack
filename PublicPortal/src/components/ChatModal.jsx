...
const ChatModal = ({ isOpen, onClose, recipientId, recipientName, recipientRole }) => {
  ...
  }, [modalOpen, user, currentRecipient.id, chatId]);
  ...
  const createOrGetChat = async () => {
    ...
    try {
      setLoading(true);
      const res = await userRequest.post('/chats', { participantId: currentRecipient.id });
  ...
  const handleSendMessage = async (e) => {
    ...
    try {
      // Send via API first
      await userRequest.post(`/chats/${chatId}/messages`, {
      ...
      <div className="bg-white rounded-lg shadow-xl w-full max-w-2xl h-3/4 flex flex-col">
        ...
        <div className="p-4 border-b border-gray-200 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="font-semibold text-gray-800">{currentRecipient.name}</h3>
              <p className="text-sm text-gray-500">{currentRecipient.role}</p>
            </div>
          </div>
          <button
            onClick={onClose}
